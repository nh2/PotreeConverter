...
namespace chunker_countsort_laszip {
	...
	struct Node {
		...
		Node(string id, int64_t numPoints) {
			this->id = id;
	...
	void distributePoints(vector<Source> sources, Vector3 min, Vector3 max, string targetDir, NodeLUT& lut, State& state, Attributes& outputAttributes, Monitor* monitor) {
		...
		vector<std::atomic_int64_t> counters(nodes.size());
	...
	NodeLUT createLUT(vector<atomic_int64_t>& grid, int64_t gridSize, double cubeSize) {
		...
		// grid has a fixed resolution), so it would become one un-splittable chunk.
		// The downstream indexer reads each whole chunk into memory, so such a cell would later OOM.
		// We use `INT32_MAX` as the limit because `NodeLUT.grid` stores node indices as `int32_t`,
		// and that value certainly counts as "far too large".
		// Hitting this limit always means the inputs are in mismatched coordinate frames,
		// producing a giant bounding box (and thus giant cells).
		// We fail fast here instead of risking OOM later.
		//
		// Note: this limit does NOT determine how many points end up in a single octree node
		// generated by Converter and loaded by Potree in the browser.
		// The chunks produced here are only the indexer's coarse input; `indexer::buildHierarchy()`
		// recursively subdivides every chunk until each final octree node has fewer
		...
		for (int64_t index = 0; index < int64_t(grid_high.size()); index++) {
			...
			if (count >= maxPointsPerCell) {
				...
				int64_t iz = index / (gridSize * gridSize);

				stringstream ss;
				ss << "Error: Too many points in a single counting cell: " << formatNumber(count) << " points." << endl;
				ss << "A single finest-level grid cell cannot hold more than " << formatNumber(maxPointsPerCell) << " points (to bound memory)." << endl;
				ss << "3d grid index: " << ix << ", " << iy << ", " << iz << endl;
				ss << "grid size: " << gridSize << "^3 cells; bounding cube edge: " << formatNumber(cubeSize, 3) << " (in the point cloud's units, usually metres)" << endl;
				ss << "=> each cell is " << formatNumber(cellSize, 3) << " wide." << endl;
				ss << "This almost always means the bounding box is far too large, typically because the input clouds are in mismatched coordinate frames (e.g. two scans placed kilometers apart)." << endl;
	...
	void doChunking(vector<Source> sources, string targetDir, Vector3 min, Vector3 max, State& state, Attributes outputAttributes, Monitor* monitor) {
		...
		{ // DISTIRBUTE
			...
			double cubeSize = (max - min).max();
			auto lut = createLUT(grid, gridSize, cubeSize);
